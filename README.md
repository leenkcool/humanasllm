...
| **FDE（前沿部署工程师）** | 2026 硅谷最火岗位 | FDE 的平台化：N 个 FDE 变成一条模型路由，详见下节 |

### FDE：前沿部署工程师的平台化

**FDE（Forward Deployed Engineer，前沿/前置部署工程师）** 源自 Palantir 的「跨界翻译官」模式——兼具技术、产业与落地能力，派驻企业现场，把 AI 模型与具体业务场景深度融合落地。FDE 是一个人，P390 把 N 个 FDE 变成调度池里一条可随时调用的模型路由。
...
**近期**
- 📱 移动端 / PWA + on-call 值班：工程师手机接单、值班轮换
- 🧑‍💻 多工程师调度：技能标签 + 负载均衡的智能分配
- 💰 计费计量：任务成本核算（人力成本可计量）
- 🛠️ 管理后台：细粒度 RBAC、审计配置化

**中期**
- 🤖 策略智能漂移：AI 能力评估自动调整 general 池边界（涉密类锁死不漂移）
- 📊 质量数据资产：人工产出反哺私有评测集/精调（合规前提下）
- 🔔 通知升级序列：on-call 逐级升级、超时告警

**长期**
- 🌐 人工能力市场：外部 Agent 生态接入，治理策略即服务
